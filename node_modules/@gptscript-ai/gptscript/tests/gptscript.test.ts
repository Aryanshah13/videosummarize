...
describe("gptscript module", () => {
    beforeAll(async () => {
        if (!process.env.REDACTED && !process.env.GPTSCRIPT_URL) {
            throw new Error("neither REDACTED nor GPTSCRIPT_URL is set")
        }
        ...
        g = new gptscript.GPTScript({APIKey: process.env.REDACTED})
    })
    afterAll(() => {
        gFirst.close()
        g.close()
    ...
    test("run executes and streams a file with global tools correctly", async () => {
        ...
        const opts = {
            disableCache: true,
            credentialOverrides: ["github.com/gptscript-ai/gateway:REDACTED"]
        }
